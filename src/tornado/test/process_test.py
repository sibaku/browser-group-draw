...
class ProcessTest(unittest.TestCase):
    ...
    def test_multi_process(self):
        ...
        with ExpectLog(gen_log, "(Starting .* processes|child .* exited|uncaught exception)"):
            ...
            try:
                userID = fork_processes(3, max_restarts=3)
                self.assertTrue(userID is not None)
            ...
            try:
                if userID in (0, 1):
                    self.assertEqual(userID, task_id())
                ...
                logging.error("exception in child process %d", userID, exc_info=True)
                raise
