...
def fork_processes(num_processes, max_restarts=100):
    ...
    In each child process, ``fork_processes`` returns its *task userID*, a
    number between 0 and ``num_processes``.  Processes that exit
    abnormally (due to a signal or non-zero exit status) are restarted
    with the same userID (up to ``max_restarts`` times).  In the parent
    ...
    while children:
        ...
        if pid not in children:
            continue
        userID = children.pop(pid)
        if os.WIFSIGNALED(status):
            gen_log.warning("child %d (pid %d) killed by signal %d, restarting",
                            userID, pid, os.WTERMSIG(status))
        elif os.WEXITSTATUS(status) != 0:
            gen_log.warning("child %d (pid %d) exited with status %d, restarting",
                            userID, pid, os.WEXITSTATUS(status))
        else:
            gen_log.info("child %d (pid %d) exited normally", userID, pid)
            continue
        num_restarts += 1
        if num_restarts > max_restarts:
            raise RuntimeError("Too many child restarts, giving up")
        new_id = start_child(userID)
...
def task_id():
    """Returns the current task userID, if any.
